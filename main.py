...
- [NO-SELL-LOGIC]: 「非推銷式進場」與「非額外支出」的視覺心理權威。

### 象限 IV：戰略賦能與篩選 (Strategic Empowerment)
- [AUDIENCE-FIT]: 適合共鳴的特定族群。
- [KEY-PROFILING]: 針對經理人/主婦/上班族/學生的切入關鍵字。
- [PARTNER-LEARNING]: 對夥伴的教學傳遞路徑。
- [GOLDEN-QUESTIONS]: 五句「庸才vs人才」識別判斷式。
...
async def root():
    ...
    <html>
        <head>
            <title>AS-CORE v3.0 | Thinking Engine</title>
            ...
            <style>
                ...
                #report { margin-top:25px; background:#000; color:#33ff33; padding:25px; text-align:left; border-left:5px solid #33ff33; min-height:300px; white-space:pre-wrap; font-size:14px; line-height:1.7; border-radius:5px; font-family:'Courier New', monospace; box-shadow: inset 0 0 20px rgba(0,255,0,0.05); }
            </style>
        </head>
        <body>
            <div class="container">
                <h1 style="margin:0;">⚡ [AS-CORE-STRATEGIST]</h1>
                <p style="color:#888; margin-bottom:25px;">數位外骨骼策略官 v3.0 | 思考型引擎裝配</p>
                <div class="panel">
                    <label style="color:#666; font-size:12px;">[ 1. 核心火力選擇 ]</label>
            ...
            <script>
                function fire(){
                    ...
                    bar.style.width = '0%';
                    report.innerText = '>>> 系統啟動...';
                    status.innerText = '📡 數據上傳 Zeabur 工廠中...';
                    ...
                    xhr.upload.onprogress = (e) => {
                        if (e.lengthComputable) {
                            ...
                            bar.style.width = p + '%';
                            if(p === 100) status.innerText = '🧠 數據抵達，' + (model.includes('thinking') ? '思考型引擎深度演算中 (預計 15-40 秒)...' : 'Gemini 演算中...');
                        }
                    };
                    xhr.onload = function() {
                        try {
                            ...
                            status.innerText = data.status === 'success' ? '✅ 戰略審計報告已封裝' : '⚠️ 任務中斷';
                        } catch(e) { report.innerText = '❌ 系統回傳異常'; }
                        btn.disabled = false;
                    };
...
async def analyze_audio(file: UploadFile = File(...), model_type: str = Form(...)):
    try:
        ...
        # [暴力修復] 強制對齊 models/ 前綴
        clean_name = model_type.split('/')[-1]
        target_model = f"models/{clean_name}"
        ...
        return {"analysis": f"❌ 運算崩潰 [System Error]: {str(e)}", "status": "error"}
